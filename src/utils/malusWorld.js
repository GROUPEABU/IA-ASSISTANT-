...
// Ley 38/1992 art. 70 — épígrafes par seuils INCLUSIFS (≤) :
// ≤120 → 0% · ≤160 → 4,75% · ≤200 → 9,75% · >200 → 14,75%
...
// ─── 🇩🇰 DANEMARK — registreringsafgift (valeur + CO₂, base 2025) ───────
// La taxe danoise est DOMINÉE par la composante valeur (25 % / 85 % / 150 %
// par tranches de prix), à laquelle s'ajoute un tillæg CO₂ et des déductions.
// DKK→EUR @0,134. Réf : Lov om registreringsafgift §3-§5a.
const DKK = 0.134;
const computeDKValue = (priceEur, fuelType) => {
  // Conversion prix € → DKK (la base légale est en DKK)
  const priceDKK = priceEur / DKK;
  // Barème valeur 2025 : 25 % ≤ 71 800 DKK, 85 % de 71 800 à 223 400, 150 % au-delà
  let tDKK = 0;
  tDKK += Math.min(priceDKK, 71800) * 0.25;
  if (priceDKK > 71800) tDKK += (Math.min(priceDKK, 223400) - 71800) * 0.85;
  if (priceDKK > 223400) tDKK += (priceDKK - 223400) * 1.50;
  return tDKK; // en DKK (déductions appliquées dans le case)
};
const computeDKco2 = g => {
  // Tillæg CO₂ 2025 : 280 DKK/g ≤117, 560 DKK/g 117-150, 950 DKK/g >150
  if (g <= 0) return 0;
  let dkk = Math.min(g, 117) * 280;
  if (g > 117) dkk += (Math.min(g, 150) - 117) * 560;
  if (g > 150) dkk += (g - 150) * 950;
  return dkk; // en DKK
};
...
function buildCountryData(code, g, kg = 1500, fuelType = "thermique", dateImmat = "2025-03-12", isImported = false, extra = {}) {
  ...
  switch (code) {
    ...
    case "GB":
      {
        ...
        // Expensive Car Supplement (ECS) : supplément annuel pendant 5 ans (années
        // 2 à 6) si prix catalogue > £40 000. EV inclus depuis 01/04/2025.
        const ecsRate = { "2024": 410, "2025": 425, "2026": 440 }[ukPeriod] || 425;
        const priceGBP = vehiclePrice / 1.17; // € → £
        const ecsTotal = priceGBP > 40000 ? ecsRate * 5 : 0;
        const aTotal = a + ecsTotal; // VED 1ère année + ECS 5 ans
        const eur = Math.round(aTotal * 1.17);
        return {
          tax_name: `VED First-Year Rate${ecsTotal ? " + ECS" : ""} — Barème ${periodLabels[ukPeriod]}`,
          threshold_gkm: 1,
          max_penalty_eur: Math.round((5690 + 440 * 5) * 1.17),
          currency_symbol: "£",
          ...
          specific_penalty: `£${aTotal.toLocaleString("en-GB")} (~${eur} €)`,
          specific_penalty_amount: eur,
          has_malus: aTotal > 10,
          severity: sev(eur),
          notes: `Date d'immat. = ${formatDateFR(dateImmat)}. Barème ${ukPeriod}/27. VED 1ère année £${a.toLocaleString("en-GB")}${ecsTotal ? ` + Expensive Car Supplement £${ecsRate}/an × 5 ans = £${ecsTotal.toLocaleString("en-GB")} (prix > £40 000)` : ""}.`,
          source: "gov.uk",
    ...
    case "ES":
      {
        ...
        return {
          ...
          max_penalty_eur: null,
          currency_symbol: "€",
          system_description: `Ley 38/1992 (Impuestos Especiales) art. 70 - Taxe immatriculation = % de la valeur HT, selon CO₂ (seuils inclusifs ≤). Région : ${regionLabel}.${isImported ? ` Importé : décote ${decoteES}% (tabla anexo).` : ''}`,
          brackets: [{
            min_gkm: 0,
            max_gkm: 120,
            penalty: "0%",
    ...
    case "AT":
      {
        const isEVat = fuelType === "ev";
        const penalty = isEVat ? 0 : computeAT(g);
        // NoVA % calc — formule 2025 : (CO₂ − 112) ÷ 5, plafond 80%
        const novaPct = isEVat ? 0 : Math.min(80, Math.max(0, Math.round((g - AT_CO2_THRESHOLD) / 5)));
    ...
    case "DK":
      {
        // EV : abattement spécifique (bundfradrag 165 500 DKK + fradrag grøn),
        // phase-in progressif jusqu'en 2035 → quasi nul à faible valeur.
        const isEVdk = fuelType === "ev";
        const valDKK = isEVdk ? 0 : computeDKValue(vehiclePrice, fuelType);
        const co2DKK = isEVdk ? 0 : computeDKco2(g);
        // Déduction (bundfradrag) 2025 : 33 600 DKK pour thermique
        const deductionDKK = isEVdk ? 0 : 33600;
        const totalDKK = Math.max(0, valDKK + co2DKK - deductionDKK);
        const a = Math.round(totalDKK * DKK); // DKK → EUR
        return {
          tax_name: "Registreringsafgift",
          threshold_gkm: 1,
          max_penalty_eur: null,
          currency_symbol: "€",
          system_description: `Registreringsafgift = la + élevée d'Europe. Valeur véhicule : 25% (≤71 800 DKK) + 85% (71-223 kDKK) + 150% (>223 kDKK) + tillæg CO₂ (280→950 kr/g) − bundfradrag 33 600 DKK. Prix : ${vehiclePrice.toLocaleString()} €.`,
          brackets: [{
            min_gkm: 0,
            max_gkm: 0,
          ...
          specific_penalty: isEVdk ? "~0 € (EV, abattement)" : `~${a.toLocaleString()} € (valeur + CO₂)`,
          specific_penalty_amount: a,
          has_malus: a > 0,
          severity: sev(a),
          notes: `Valeur ~${Math.round(valDKK*DKK).toLocaleString()} € + CO₂ ~${Math.round(co2DKK*DKK).toLocaleString()} € − bundfradrag 33 600 DKK. Tarifs 2025. EV : bundfradrag 165 500 DKK + fradrag grøn 45 000 DKK (quasi exonéré). Réf: Lov om registreringsafgift §3-§5a.`,
    ...
    case "IE":
      { const b=[{max:50,r:7},{max:80,r:9},{max:110,r:12.5},{max:130,r:17},{max:155,r:23},{max:190,r:30},{max:230,r:36},{max:999,r:41}];
        ...
        const vrt = Math.round(vBrut * (1 - decoteIE / 100));
        // Prélèvement NOx (séparé du VRT) — barème €5/€15/€25 par mg/km, plafonné.
        // Le NOx (mg/km) n'étant pas saisi, on estime une valeur Euro 6d typique
        // par carburant (essence ~20 mg, diesel ~60 mg) ; nul pour les BEV.
        const noxMg = isEVie ? 0 : (fuelKind === "diesel" ? 60 : 20);
        const computeNOxIE = (mg) => {
          if (mg <= 0) return 0;
          let lv = 0;
          lv += Math.min(mg, 40) * 5;
          if (mg > 40) lv += (Math.min(mg, 80) - 40) * 15;
          if (mg > 80) lv += (mg - 80) * 25;
          const cap = fuelKind === "diesel" ? 4850 : 600;
          return Math.min(lv, cap);
        };
        const noxLevy = computeNOxIE(noxMg);
        const v = vrt + noxLevy;
    ...
    case "IS": return {tax_name:"Vörugjald (droit d'accise CO₂)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"ISK",system_description:"Droit d'accise à l'importation indexé sur le CO₂ (g/km WLTP) — taux croissant avec les émissions, intégré au prix de vente. Taxe annuelle (bifreiðagjald) également liée au CO₂. Montants non modélisés ici.",brackets:[{min_gkm:0,max_gkm:999,penalty:"Variable CO₂",label:"Accise à l'import"}],exemptions:["EV : accise réduite/nulle, mais frais kilométriques (kílómetragjald) depuis 2024"],specific_penalty:"Selon CO₂ (intégré au prix)",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"L'Islande taxe le CO₂ à l'import ET annuellement, mais via le prix — pas de malus séparé affichable.",source:"Skatturinn (administration fiscale islandaise)",source_url:"https://www.skatturinn.is/atvinnurekstur/virdisaukaskattur-og-onnur-gjold/vorugjald-af-okutaekjum/",legal_ref:"Lög nr. 29/1993 um vörugjald af ökutækjum · Lög nr. 39/1988 (bifreiðagjald)",reliability:"info"};
    case "TR": return {tax_name:"ÖTV (taxe spéciale de consommation)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"₺",system_description:"Pas de malus CO₂. ÖTV de 45% à 220% selon cylindrée ET prix de base, + TVA 20%. Taxe annuelle MTV selon cylindrée et âge.",brackets:[{min_gkm:0,max_gkm:999,penalty:"45–220% du prix",label:"Selon cylindrée/prix"}],exemptions:["EV : ÖTV réduite (10–60% selon puissance kW et prix)"],specific_penalty:"Pas de malus CO₂ (ÖTV cylindrée/prix)",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Une des fiscalités auto les plus lourdes au monde, mais assise sur cylindrée et prix, pas le CO₂.",source:"Gelir İdaresi Başkanlığı (administration fiscale turque)",source_url:"https://www.gib.gov.tr",legal_ref:"Özel Tüketim Vergisi Kanunu No 4760 (liste II) · MTV Kanunu No 197",reliability:"info"};
    case "IL": return {tax_name:"Taxe d'achat « score vert »",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"₪",system_description:"Taxe d'achat de base ~83% du prix, réduite selon le « score vert » (ציון ירוק) calculé sur les émissions (CO₂, NOx, particules). Plus le véhicule émet, moins la réduction est forte — effet malus intégré au prix.",brackets:[{min_gkm:0,max_gkm:999,penalty:"83% − réduction verte",label:"Selon score vert"}],exemptions:["EV : taux réduit (relevé progressivement — 35% en 2024, hausse programmée)"],specific_penalty:"Intégré au prix (score vert)",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Système CO₂-dépendant mais fusionné dans la taxe d'achat — pas de montant séparé.",source:"רשות המסים (administration fiscale israélienne)",source_url:"https://www.gov.il/he/departments/israel_tax_authority",legal_ref:"צו תעריף המכס · Green Tax Reform 2009 (révisée 2023)",reliability:"info"};
    case "TH": return {tax_name:"Accise automobile par bandes CO₂",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"฿",system_description:"Depuis 2016, l'accise voiture est assise sur des bandes de CO₂ (g/km) : taux croissant avec les émissions, appliqué au prix usine — effet malus intégré au prix de vente. Restructuration pro-EV en cours (2026+).",brackets:[{min_gkm:0,max_gkm:999,penalty:"% du prix selon bande CO₂",label:"Accise CO₂"}],exemptions:["EV : accise fortement réduite (programme EV 3.5)"],specific_penalty:"Intégré au prix (bandes CO₂)",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Vrai système CO₂ mais en % du prix usine — pas de montant forfaitaire affichable.",source:"Excise Department (Thaïlande)",source_url:"https://www.excise.go.th",legal_ref:"Excise Act B.E. 2560 (2017) · Notification on automobile excise rates (CO₂ bands)",reliability:"info"};
    case "ID": return {tax_name:"PPnBM (taxe de luxe liée CO₂)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"Rp",system_description:"Depuis octobre 2021, la taxe de luxe PPnBM (15–70%) dépend de la consommation/des émissions CO₂ et de la cylindrée — effet malus intégré au prix.",brackets:[{min_gkm:0,max_gkm:999,penalty:"15–70% du prix",label:"Selon CO₂/cylindrée"}],exemptions:["EV : PPnBM 0% (programme véhicules électriques)"],specific_penalty:"Intégré au prix (PPnBM)",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Système CO₂-dépendant en % du prix — pas de montant séparé.",source:"Direktorat Jenderal Pajak (Indonésie)",source_url:"https://www.pajak.go.id",legal_ref:"PP No 73/2019 et PP No 74/2021 (PPnBM kendaraan bermotor)",reliability:"info"};
    case "NZ": return {tax_name:"Pas de taxe CO₂ (Clean Car Discount supprimé)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"NZ$",system_description:"Le Clean Car Discount (bonus/malus CO₂ à l'immatriculation) a été supprimé au 31/12/2023. Plus aucune taxe CO₂ à l'achat.",brackets:[{min_gkm:0,max_gkm:999,penalty:"0 NZ$",label:"Pas de malus"}],exemptions:["EV : payent les Road User Charges depuis avril 2024 (fin de l'exemption)"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Bonus/malus CO₂ actif de juillet 2021 à décembre 2023, abrogé par le gouvernement élu fin 2023.",source:"Waka Kotahi NZ Transport Agency",source_url:"https://www.nzta.govt.nz/vehicles/clean-car-programme/",legal_ref:"Land Transport (Clean Vehicles) Act 2022 · abrogation Clean Car Discount au 31/12/2023",reliability:"info"};
    case "MX": return {tax_name:"ISAN (taxe sur autos neuves)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"MX$",system_description:"Pas de taxe CO₂. ISAN progressive selon le PRIX du véhicule neuf (jusqu'à ~17%), + tenencia annuelle selon État.",brackets:[{min_gkm:0,max_gkm:999,penalty:"Selon prix",label:"ISAN"}],exemptions:["EV/hybrides : exonérés d'ISAN (loi fédérale)"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Fiscalité assise sur le prix, pas les émissions.",source:"SAT (Servicio de Administración Tributaria)",source_url:"https://www.sat.gob.mx",legal_ref:"Ley Federal del ISAN (1996, révisée annuellement)",reliability:"info"};
    case "AE": return {tax_name:"Pas de taxe CO₂ (Émirats)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"AED",system_description:"Aucune taxe CO₂ ni malus. 5% de droits de douane + 5% TVA à l'import, frais d'immatriculation fixes par émirat.",brackets:[{min_gkm:0,max_gkm:999,penalty:"0 AED",label:"Pas de malus"}],exemptions:["EV : incitations locales (recharge, stationnement) selon émirat"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Fiscalité auto quasi nulle — marché d'import attractif.",source:"Federal Tax Authority (UAE)",source_url:"https://tax.gov.ae",legal_ref:"GCC Common Customs Law (5%) · Federal Decree-Law No 8/2017 (TVA)",reliability:"info"};
    case "MA": return {tax_name:"Pas de malus CO₂ (Maroc)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"DH",system_description:"Pas de malus CO₂. Vignette annuelle (TSAV) selon puissance fiscale (CV) et carburant ; droits d'importation selon origine (0% UE si règles d'origine) + TVA 20%.",brackets:[{min_gkm:0,max_gkm:999,penalty:"Selon CV",label:"Vignette annuelle"}],exemptions:["EV/hybrides : vignette réduite"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Marché export pertinent — fiscalité assise sur la puissance fiscale, pas le CO₂.",source:"DGI Maroc (Direction Générale des Impôts)",source_url:"https://www.tax.gov.ma",legal_ref:"Code Général des Impôts art. 179 et s. (TSAV) · Accord d'association UE-Maroc",reliability:"info"};
    case "TN": return {tax_name:"Pas de malus CO₂ (Tunisie)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"DT",system_description:"Pas de malus CO₂. Taxe de circulation (vignette) selon puissance fiscale (CV) ; importation très taxée (droits + consommation) sauf régime FCR (résidents de retour).",brackets:[{min_gkm:0,max_gkm:999,penalty:"Selon CV",label:"Vignette"}],exemptions:["Régime FCR : franchise partielle à l'import (conditions strictes)"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Fiscalité assise sur la puissance fiscale, pas le CO₂.",source:"Ministère des Finances (Tunisie)",source_url:"http://www.finances.gov.tn",legal_ref:"Code de la fiscalité locale · Loi de finances (vignette CV)",reliability:"info"};
    case "DZ": return {tax_name:"Pas de malus CO₂ (Algérie)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"DA",system_description:"Pas de malus CO₂. Taxe sur les véhicules neufs selon motorisation et prix, vignette annuelle selon puissance et âge ; import de véhicules <3 ans ré-autorisé (décret 2023).",brackets:[{min_gkm:0,max_gkm:999,penalty:"Selon motorisation/prix",label:"Taxe VN + vignette"}],exemptions:["EV : exonération de la taxe véhicules neufs"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Fiscalité assise sur motorisation/prix, pas le CO₂.",source:"DGI Algérie (Direction Générale des Impôts)",source_url:"https://www.mfdgi.gov.dz",legal_ref:"Loi de finances (taxe véhicules neufs, vignette) · Décret 23-74 (import <3 ans)",reliability:"info"};
    case "UA": return {tax_name:"Accise à l'import (Ukraine)",threshold_gkm:null,max_penalty_eur:null,currency_symbol:"₴",system_description:"Pas de composante CO₂. Accise à l'importation basée sur cylindrée, carburant et âge du véhicule, + droits 10% + TVA 20%.",brackets:[{min_gkm:0,max_gkm:999,penalty:"Selon cylindrée/âge",label:"Accise import"}],exemptions:["EV : accise 1 €/kWh de batterie, TVA et droits 0% (prolongé)"],specific_penalty:"Pas de malus CO₂",specific_penalty_amount:0,has_malus:false,severity:"none",notes:"Fiscalité assise sur cylindrée et âge, pas le CO₂. Fortes incitations EV.",source:"Державна податкова служба (administration fiscale ukrainienne)",source_url:"https://tax.gov.ua",legal_ref:"Податковий кодекс України art. 215 (акцизний податок)",reliability:"info"};
    default:
      return null;
  }
}
...
// EUROPE élargie / MOYEN-ORIENT — info
{
  code: "IS", name: "Islande", flag: "🇮🇸", official: true, reliability: "info"
}, {
  code: "TR", name: "Turquie", flag: "🇹🇷", official: true, reliability: "info"
}, {
  code: "UA", name: "Ukraine", flag: "🇺🇦", official: true, reliability: "info"
}, {
  code: "IL", name: "Israël", flag: "🇮🇱", official: true, reliability: "info"
}, {
  code: "AE", name: "Émirats arabes unis", flag: "🇦🇪", official: true, reliability: "info"
},
// AFRIQUE DU NORD — marchés export Autobuyunion
{
  code: "MA", name: "Maroc", flag: "🇲🇦", official: true, reliability: "info"
}, {
  code: "TN", name: "Tunisie", flag: "🇹🇳", official: true, reliability: "info"
}, {
  code: "DZ", name: "Algérie", flag: "🇩🇿", official: true, reliability: "info"
},
// ASIE / OCÉANIE / AMÉRIQUES — info
{
  code: "TH", name: "Thaïlande", flag: "🇹🇭", official: true, reliability: "info"
}, {
  code: "ID", name: "Indonésie", flag: "🇮🇩", official: true, reliability: "info"
}, {
  code: "NZ", name: "Nouvelle-Zélande", flag: "🇳🇿", official: true, reliability: "info"
}, {
  code: "MX", name: "Mexique", flag: "🇲🇽", official: true, reliability: "info"
},
// WORLD — vrai malus CO₂
{
  code: "ZA",
